...
class Worker(Log):
    ...
    def start(self):
        ...
        # LOGGING
        SMS_LAST = get_mes(password)
        self.web_driver.find_elements_by_tag_name('input')[0].send_keys(password['user_name'])
        ...
        if self.type_sleep == 'last':
            ...
            while 1:
                if time.time() - start > MAX_TIME_WAIT:
                    print('A lot of time find element')
                    raise Exception
                try:
    ...
    def plan(self, line: Line):
        ...
        @self.error_checker
        def choose_plans():
            plans_type = ts(0.5, lambda: wd.find_elements_by_class_name('plan-type'))
            plans_type[PLAN_TYPE].find_element_by_xpath('..').click()

        # CARD
        @self.error_checker
        def button_card():
            all_cards = ts(0.5, lambda: wd.find_elements_by_class_name('card-title'))
            for ci in all_cards:
        ...
        def check_on_reg():
            if i == 0:
                # LEFT_BUTTON
                dialog = ts(2, lambda: wd.find_element_by_tag_name('client-eligibility'))
                dialog.find_elements_by_tag_name('h2')[1].click()

                # CHECK ALREADY REG
                dialog = wd.find_element_by_tag_name('client-eligibility')
                dialog.find_element_by_tag_name('input').send_keys(line.PASSPORT)
                ...
                # error
                try:
                    if wd.find_element_by_class_name('invalid-feedback').text != '':
                        type_user = 'error'
                except:
                    pass
                ...
                # already
                try:
                    if len(wd.find_elements_by_name('clientPhoneNumber')) == 1:
                        # clientPhoneNumber
                        type_user = 'already'
                except:
                    pass
                ...
                # button
                try:
                    if 'ללקוח זה נמצאה הזמנה פעילה, האם ברצונך להמשיך?' in wd.find_element_by_tag_name(
                ...
                # new
                try:
                    if len(wd.find_elements_by_name('clientId')) != 1:
                        type_user = 'new'
                except:
                    pass
                ...
                if type_user == 'already':
                    wd.find_element_by_name('clientPhoneNumber').send_keys(line.NUMBER_USER)
                    wd.find_element_by_name('clientLastFourDigits').send_keys(line.CARD[-4:])
                    dialogs = wd.find_element_by_tag_name('client-eligibility')
        ...
        @self.error_checker
        def marking_options():
            options = ts(2, lambda: wd.find_elements_by_class_name('desing-radios'))
            for u in options:
        ...
        @self.error_checker
        def enter_number():
            table = ts(0.5, lambda: wd.find_element_by_tag_name('portability-comp'))
            labels = table.find_elements_by_tag_name('label')
            if TYPE_USER == 'new' and NUMBER != 'חדש':
                try:
                    labels[1].click()
                    table.find_element_by_name('phone').send_keys(NUMBER)
                    table.find_element_by_tag_name('button').click()
                    table = wd.find_element_by_tag_name('portability-comp')
                    table = ts(2, lambda: wd.find_element_by_tag_name('portability-comp'))
                    table.find_elements_by_name('verifyNeeded')[1].find_element_by_xpath('..').click()
                    ts(2, can_skip = True)
                except:
                    pass
                # enter users data
                table.find_element_by_name('firstName').send_keys(NUMBER)
                table.find_element_by_name('lastName').send_keys(line.SURNAME)
                table.find_element_by_name('userId').send_keys(line.PASSPORT)
        ...
        def mark_last_params():
            table = ts(0.5, lambda: wd.find_element_by_tag_name('regulations-comp'))
        ...
        @self.error_checker
        def end_action():
            table = ts(0.5, lambda: wd.find_element_by_tag_name('purchase-summary'))
            ...
            if i + 1 == line.COUNT_LINES:
                ...
                if PLAN_TYPE == line.PLANS[i + 1].values():
                    ...
                    options = wd.find_elements_by_class_name('desing-radios')
                    for u in options:
        ...
        @self.error_checker
        def enter_email():
            table = ts(2, lambda: wd.find_element_by_tag_name('interactive-forms'))
            lbs = table.find_elements_by_tag_name('label')
            for uu in lbs:
                if 'ללא שליחת טפסים מקדימים' in uu.text:
                    uu.click()
                    break
            wd.find_element_by_class_name('continue').click()

        # MAILING DATA
        @self.error_checker
        def enter_mailing_data():
            table = ts(2, lambda: wd.find_element_by_tag_name('app-personaldetails'))
            if table.find_element_by_name('Phone').get_attribute('value') == '':
                table.find_element_by_name('Phone').send_keys(line.NUMBER_USER)
            ...
            table.find_element_by_name('FirstName').send_keys(line.NAME)
            table.find_element_by_name('LastName').send_keys(line.SURNAME)

        # CITY,ADDR
        @self.error_checker
        def enter_addr():
            table = ts(0.5, lambda: wd.find_element_by_tag_name('app-personaldetails'))
            for ii in range(2):
                if ii == 0:
                    tag = 'City'
                    parametr = line.CITY
        ...
        def enter_more_data():
            ...
            if table.find_element_by_name('HouseNumber').get_attribute('value') == '':
                table.find_element_by_name('HouseNumber').send_keys(line.HOUSE_NUMBER)

            if table.find_element_by_name('ApartmentNumber').get_attribute('value') == '':
                table.find_element_by_name('ApartmentNumber').send_keys(line.APART_NUMBER)

            if table.find_element_by_name('POBox').get_attribute('value') == '':
                table.find_element_by_name('POBox').send_keys(line.POBox)
            wd.find_element_by_class_name('continue').click()

        # DROP CITY,ADDR
        @self.error_checker
        def enter_more_data2():
            table = ts(2, lambda: wd.find_element_by_tag_name('retail-deliverytype'))
            for ii in range(2):
                if ii == 0:
                    tag = 'City'
                    parameter = line.CITY
            ...
            ts(2, lambda: wd.find_element_by_class_name('continue')).click()

        # CARD
        @self.error_checker
        def enter_card():
            table = ts(1, lambda: wd.find_element_by_tag_name('payemnt-comp'))
            if TYPE_USER == 'already':
                table = wd.find_element_by_tag_name('payemnt-comp')
                table.find_element_by_tag_name('label').click()

            if line.TYPE_PAY == 'card':
                table.find_element_by_name('creditCardNumber').send_keys(line.CARD)
                table.find_element_by_name('creditCardHolderId').send_keys(line.PASSPORT)

            else:
                table.find_element_by_name('bankOrderClientId').send_keys(line.CARD)
                table.find_element_by_name('bankOrderFirstName').send_keys(line.CARD)
                table.find_element_by_name('bankOrderLastName').send_keys(line.CARD)
        ...
        @self.error_checker
        def save():
            ts(1, lambda: wd.find_element_by_class_name('continue')).click()
            boxes = ts(1, lambda: wd.find_elements_by_class_name('box'))
