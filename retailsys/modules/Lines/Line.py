from config import *
from modules.data import MainSheet
import re
...
class Line:
    ...
    def getattr(self):
        def get_data(name, offset=0):
            return MainSheet.get_value(LINE_MATCHER[name], self.index + offset)

        return get_data

    def setattr(self):
        def set_data(name, value, offset=0):
            return MainSheet.set_value(LINE_MATCHER[name], self.index + offset, str(value))
    ...
    def __init__(self, index):
        ...
        print(self.index, self.COUNT_LINES)
        self.PLANS = [
            {
