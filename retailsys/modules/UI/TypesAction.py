...
class Error(Log):
    ...
    def wait_change_position(self):
        ...
        wait(lambda: self.position == start_position)
        return self.type
...
class Stack:
    ...
    def force_line(self, line_index):
        if line_index in self.__end_indexes:
            self.__end_indexes.pop(self.__end_indexes.index(line_index))
    ...
    def __reload_lines_ui(self):
        for i in self.__linesUI.values():
            i.destroy()
        for i in self.__destroy:
            i.destroy()

        self.__linesUI = {}
        # active
        if self.__run_index != -1:
            self.__destroy.append(self.__place_line(None, "run"))
            self.__add_line_in_ui(line_index = self.__run_index)
            self.__destroy.append(self.__place_line(None, ''))
        ...
        # wait
        self.__destroy.append(self.__place_line(None, "wait"))
        ...
        # end
        self.__destroy.append(self.__place_line(None, "end"))
